...
describe('admin visual action guardrails', () => {
  ...
  it('uses the shared Button component for ability list base optimization actions', () => {
    ...
    expect(abilityListBasePage).toContain('<Button variant="secondary" @click="openManualSuggestionSheet">人工补充建议</Button>')
    expect(abilityListBasePage).toContain('activeWorkspaceTab')
    ...
    expect(abilityListBasePage).toContain('analysisState')
    expect(abilityListBasePage).toContain('暂无优化建议')
    expect(abilityListBasePage).toContain('选择左侧来源或调整问题类型筛选，暂无匹配建议。')
    expect(abilityListBasePage).toContain('当前没有选中的建议')
    ...
    expect(abilityListBasePage).not.toContain('<AdminTableColumn label="操作"')
    expect(abilityListBasePage).not.toContain("handleSuggestionAction('view', row)")
    expect(abilityListBasePage).not.toContain("handleSuggestionAction('adopt', row)")
    expect(abilityListBasePage).not.toContain("handleSuggestionAction('defer', row)")
    expect(abilityListBasePage).not.toContain("handleSuggestionAction('reject', row)")
    expect(abilityListBasePage).not.toContain('class="operation-message"')
