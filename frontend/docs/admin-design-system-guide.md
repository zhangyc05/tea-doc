...
- 能力画像教师详情页是观察当前状态的旗舰页，不是证据链审计页。首屏必须呈现当前画像和雷达观察，不以教师评语、最终评价或审计结论作为首屏中心。
- 教师详情页雷达图语义必须稳定：中心为基本能力，状态只允许“达标 / 未达标”；外圈四维为教学能力、教研能力、实践能力、服务能力；层级只写新手、胜任、骨干、名师，不写“区”。
- 教师详情页雷达下方模块固定为能力结构、目标对照摘要、适合发展方向、形成依据。形成依据只作为二级说明，说明当前画像如何形成，不命名为证据链，不上升为审计入口。
- 教师详情页文案避免判断性和审计化措辞，包括“能力不足、短板、弱项、落后、不达标、重点帮扶、评价结论、证据链、新手区、胜任区、骨干区、名师区”。表达基本能力状态时只能使用“未达标”，不用“不达标”。
- 教师详情页的依据动作必须进入已存在页面或明确降级，不允许空点击。当前引用记录进入 `/admin/archive/teacher/:teacherId`，能力清单口径进入 `/admin/ability-list/execution`。
