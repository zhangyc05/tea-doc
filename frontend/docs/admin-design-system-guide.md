...
- 能力清单基准模板页顶部“优化基准模板 / 查看版本记录 / 派生执行版”已迁移到公共 `Button`。
- “优化基准模板”使用默认主按钮，“查看版本记录”使用 `outline`，“派生执行版”保留轻量文字入口并使用 `ghost`；页面仅保留 `primary-action`、`secondary-action`、`derive-action` 结构类承接 hero 区排版。
- 该批次只替换基准模板页 hero 主流程入口，不改变 `goToOptimization()`、`goToVersionHistory()`、`deriveExecutionVersion()` 的跳转、版本抽屉和派生执行版行为。

F4-34 试点迁移：

- 能力清单执行版页顶部“派生下一周期执行版 / 历史版本”已迁移到公共 `Button`。
- “派生下一周期执行版”使用默认主按钮并保留原图标，“历史版本”使用 `outline`；页面仅保留 `primary-action`、`secondary-action` 结构类承接 hero 区图标和排版。
- 该批次只替换执行版页 hero 主流程入口，不改变 `deriveNextVersion()`、`openVersionDrawer()` 的派生执行版跳转和历史版本抽屉行为。
...
| 复杂编辑表单 | 字段分组、映射关系、删除动作 | 640-720px；如要求映射 660px | 分组区块明确；危险动作可在底部左侧，确认动作在右侧 |
...
- 如果本次只改 store、domain、mock service 或测试，不影响页面结构，台账可写“结构不变”，但仍要说明涉及页面入口是否变化。
- 如果本次改动会改变效果图结构、比例或密度，必须先在台账标为“结构调整”，说明原因和受影响页面，不能直接标记为无风险迁移。
