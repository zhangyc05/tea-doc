...
- 优化建议已合并到基准模板页；制度文件、运行反馈和人工建议在基准模板页内进入建议列表，采纳后只形成待应用项，应用后转为基准模板修订草稿，不直接改正式基准模板。旧优化路由只作为兼容入口，跳回基准模板页的优化建议区域。
- 执行版页负责派生下一周期执行版；发布确认页不得自动派生版本，只消费执行版页已有的派生结果并执行发布。
- 执行版指标编辑先形成待确认调整，表格可展示草稿态；必须通过“确认调整”写入正式执行版，或通过“撤回调整”丢弃。
- 岗位/聘期要求映射页左侧要求对象必须驱动中间表格、详情、统计和新增项归属，禁止切换对象后仍显示同一组映射数据。
...
- 能力清单执行版页顶部“派生下一周期执行版 / 历史版本”已迁移到公共 `Button`。
- “派生下一周期执行版”使用默认主按钮并保留原图标，“历史版本”使用 `outline`；待确认调整出现时额外显示“确认调整”和“撤回调整”。
- 执行版编辑指标保存后进入待确认调整，不直接写入正式执行版；`deriveNextVersion()` 仍负责派生并跳转发布确认页。
...
- 能力清单基准模板页内优化建议工作区“上传制度文件 / 重新分析运行反馈 / 形成修订草稿 / 查看详情 / 采纳 / 暂缓 / 弃用 / 应用”已迁移到公共 `Button`。
- 工作区工具栏使用默认和 `secondary`，表格行内动作使用 `ghost`、默认、`secondary`、`danger` 的 `sm` 尺寸；旧优化路由保留兼容提示并跳转到基准模板页 `?optimization=1`。
- 该批次只替换优化建议真实动作按钮，不改变 `uploadPolicy()`、`rerunAnalysis()`、`applyToBaseTemplate()`、`handleSuggestionAction()` 的上传制度来源、重新分析、应用建议和建议状态流转行为。
